...
#define MAX_BUFFER_SIZE 500    // Размер буфера
#define MIN_RSSI -120          // Минимальный уровень сигнала WiFi (dBm)
...
// ==== ДЛЯ ГЕРКОНА (ПРЕРЫВАНИЕ) ====
volatile bool reedChanged = false;
volatile int reedState = 0;
unsigned long lastInterruptTime = 0; // антидребезг

void IRAM_ATTR reedISR() {
  unsigned long currentTime = millis();
  if (currentTime - lastInterruptTime > 50) { // 50 мс антидребезг
...
void setup() {
  ...
  // Подключаем прерывание на геркон
  attachInterrupt(digitalPinToInterrupt(REED_SWITCH_PIN), reedISR, CHANGE);
...
void loop() {
  ...
  // === Реакция на изменение геркона ===
  if (reedChanged) {
    reedChanged = false;

    Serial.printf("Изменение геркона! Новое состояние: %d\n", reedState);

    sensors.requestTemperatures();
    float temperature = sensors.getTempCByIndex(0);

    if (temperature != DEVICE_DISCONNECTED_C && temperature <= 84.0) {
      addDataToBuffer(device_id, client_id, temperature, reedState, 1);
    } else {
      addDataToBuffer(device_id, client_id, 0.0, reedState, 0);
    }

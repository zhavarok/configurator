...
class AttendanceSystemConfigurator:
    def __init__(self, root):
        self.window = Toplevel(root)
        self.window.title("Конфигурация Системы учета времени")
        ...
        image_path = resources.get_image_path('clock.png')
        ...
        self.tooltip_label.bind("<Enter>",
                                lambda e: self.show_tooltip(e, "Настройте параметры для системы учета времени."))
        ...
        self.firmware_path = tk.StringVar(value=self.get_firmware_path('esp32_attendance.bin'))
        ...
        self.ssid_label.grid(column=0, row=1, padx=10, pady=10)
        self.ssid_entry = ttk.Entry(settings_frame, width=20)
        self.ssid_entry.grid(column=1, row=1, padx=10, pady=10)
        self.set_ssid_button = ttk.Button(settings_frame, text="Установить", command=self.set_ssid)
        self.set_ssid_button.grid(column=2, row=1, padx=10, pady=10)
        ...
        self.set_password_button.grid(column=2, row=2, padx=10, pady=10)

        self.url_label = ttk.Label(settings_frame, text="URL для отправки данных:")
        self.url_label.grid(column=0, row=3, padx=10, pady=10, sticky='w')
        self.url_entry = ttk.Entry(settings_frame, width=40)
        self.url_entry.grid(column=1, row=3, padx=10, pady=10, sticky='ew')
        self.set_url_button = ttk.Button(settings_frame, text="Установить", command=self.set_url)
        self.set_url_button.grid(column=2, row=3, padx=10, pady=10)

        self.get_settings_button = ttk.Button(settings_frame, text="Считать настройки", command=self.get_settings)
        self.get_settings_button.grid(column=0, row=4, columnspan=3, padx=10, pady=10, sticky='ew')
    ...
    def flash_esp32(self):
        ...
        firmware_file = self.firmware_path.get() or self.get_firmware_path('esp32_attendance.bin')
        ...
        if not firmware_file or not os.path.exists(firmware_file):
            messagebox.showerror("Ошибка", f"Файл прошивки не найден: {firmware_file}. Убедитесь, что файл добавлен в сборку или выберите другой файл.")
        ...
        # Копируем файл прошивки во временную папку
        ...
        def flash():
            ...
            try:
                ...
                # Формируем аргументы для esptool
                args = [
    ...
    def get_firmware_path(self, firmware_name='esp32_attendance.bin'):
    ...
    def set_url(self):
        ...
        if url and url.startswith("http://"):
            ...
            messagebox.showerror("Ошибка", "Введите корректный URL (например, http://80.80.101.123:502/data).")
            logging.error("Некорректный формат URL")

    def get_settings(self):
        if self.serial_connection and self.serial_connection.is_open:
            command = "gettimesettings\n"
            self.serial_connection.write(command.encode('utf-8'))
            self.response_text.insert(tk.END, "Запрос настроек системы учета времени отправлен.\n")
            logging.debug("Отправлен запрос gettimesettings")
        else:
            self.response_text.insert(tk.END, "Не подключено ни одно устройство\n")
            logging.warning("Попытка отправки gettimesettings без подключения")
...
if __name__ == "__main__":
    root = tk.Tk()
    app = AttendanceSystemConfigurator(root)
    root.mainloop()